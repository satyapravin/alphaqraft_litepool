...
#ifndef LITEPOOL_CORE_PY_LITEPOOL_H_
#define LITEPOOL_CORE_PY_LITEPOOL_H_
...
/**
 * Templated subclass of LitePool,
 * to be overrided by the real LitePool.
 */
template <typename LitePool>
class PyLitePool : public LitePool {
 public:
  using PySpec = PyEnvSpec<typename LitePool::Spec>;

  PySpec py_spec;
  static std::vector<std::string> py_state_keys;
  static std::vector<std::string> py_action_keys;

  explicit PyLitePool(const PySpec& py_spec)
      : LitePool(py_spec), py_spec(py_spec) {}
  ...
  void PySend(const std::vector<py::array>& action) {
    ...
    LitePool::Send(arr);  // delegate to the c++ api
  }
  ...
  std::vector<py::array> PyRecv() {
    std::vector<Array> arr;
    {
      py::gil_scoped_release release;
      arr = LitePool::Recv();
      DCHECK_EQ(arr.size(), std::tuple_size_v<typename LitePool::State::Keys>);
    }
    std::vector<py::array> ret;
    ret.reserve(LitePool::State::kSize);
...
template <typename LitePool>
std::vector<std::string> PyLitePool<LitePool>::py_state_keys =
    PyLitePool<LitePool>::PySpec::py_state_keys;
template <typename LitePool>
 ...
 * It will register the litepool instance to the registry.
...
#define REGISTER(MODULE, SPEC, LITEPOOL)                              \
  ...
  py::class_<LITEPOOL>(MODULE, "_" #LITEPOOL, py::metaclass(get_abc_meta())) \
      ...
      .def("_send", &LITEPOOL::PySend)                                \
      .def("_reset", &LITEPOOL::PyReset)                              \
      .def_readonly_static("_state_keys", &LITEPOOL::py_state_keys)   \
      .def_readonly_static("_action_keys", &LITEPOOL::py_action_keys); 

#endif  // LITEPOOL_CORE_PY_LITEPOOL_H_
