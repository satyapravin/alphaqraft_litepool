...
class VecNormalize:
    ...
    def load(self, path):
        ...
        self.obs_rms.mean = state['obs_mean'].to(device)
        self.obs_rms.var = state['obs_var'].to(device)
        self.obs_rms.count = state['obs_count'].to(device)
        self.ret_rms.mean = state['ret_mean'].to(device)
        self.ret_rms.var = state['ret_var'].to(device)
        self.ret_rms.count = state['ret_count'].to(device)
    ...
    def normalize_reward(self, reward):
        if not self.norm_reward:
            return reward
        reward = torch.as_tensor(reward, dtype=torch.float32, device=device)  # Shape: [num_envs]
    ...
    def step(self, actions):
        ...
        obs = torch.as_tensor(obs, dtype=torch.float32, device=device)  # Shape: [num_envs, 2420]
        rews = torch.as_tensor(rews, dtype=torch.float32, device=device)  # Shape: [num_envs]
